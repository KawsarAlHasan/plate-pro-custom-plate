...
const ForgotPassword = ({ forgotText }) => {
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      if (response.status == 200) {
        message.success(forgotText?.successMessage);
  ...
  const onFinishFailed = (errorInfo) => {
    message.error(forgotText?.errorMessage);
  };
      ...
      <div className="bg-white p-10 py-12 shadow-2xl rounded-2xl w-full max-w-[550px] border border-gray-100">
        ...
        >
          ...
          <Form.Item
            ...
            name="email"
            rules={[
              { required: true, message: forgotText?.emailRequired },
              { type: "email", message: forgotText?.emailInvalid },
            ]}
          >
            <Input
          ...
          <Form.Item className="mb-0">
            <Button
              ...
              size="large"
            >
              {loading ? forgotText?.loading : forgotText?.submit}
            </Button>
          </Form.Item>
        </Form>
